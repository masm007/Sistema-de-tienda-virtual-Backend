...
namespace Application.UseCases.Orders {
    public class GetOrderByOrderNumberForUserUseCase {
        private readonly IOrderRepository<OrderEntity, string> _orderRepository;

        public GetOrderByOrderNumberForUserUseCase(IOrderRepository<OrderEntity, string> orderRepository) {
            _orderRepository = orderRepository;
        }

        public async Task<OrderDto> ExecuteAsync(string orderNumber, int userId) {
            var ord = await _orderRepository.GetByOrderNumberForUserAsync(orderNumber, userId);
            if (ord == null) {
